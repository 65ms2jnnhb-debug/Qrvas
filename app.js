...
async function loadSEQGTFSFromFiles() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      let response = await fetch(`./SEQ_GTFS/${file}`);
    ...
    if (audioFileCache[audioPath]) {
      ...
      if (typeof window !== 'undefined' && typeof window.require !== 'undefined') {
        try {
          ...
          fileUrl = `http://localhost:3000/audio/${encodeURI(normalizedPath)}`;
          isElectron = true;
          console.log('   [Electron] Using local HTTP server for audio');
        } catch (e) {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (setRunBtn) setRunBtn.addEventListener("click", function () {
    ...
    if (stoppingPatterns[run]) {
      ...
      if (!destinationWindow || destinationWindow.closed) {
        destinationWindow = window.open('http://localhost:3000/destination.html', 'DestinationBoard', 'width=1200,height=400');
      }
  ...
  function playMindTheGap() {
    ...
    if (isAtFirstStation) {
      ...
      if (!displayWindow || displayWindow.closed) {
        displayWindow = window.open('./display.html', 'AnnouncementDisplay', 'width=900,height=200');
      }
